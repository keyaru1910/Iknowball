...
@Injectable() export class NewsScheduler implements OnModuleInit { 
  constructor(@InjectQueue(NEWS_QUEUE) private queue:Queue){} 
  async onModuleInit(){ 
    try {
      await this.queue.add('sync',{}, {jobId:'news-rss-initial',removeOnComplete:true}); 
      await this.queue.add('sync',{}, {jobId:'news-rss-30-minutes',repeat:{every:30*60*1000},removeOnComplete:{count:50},removeOnFail:{count:100}}); 
    } catch (err: any) {
      console.warn(`[NewsScheduler] Không thể kích hoạt hàng đợi RSS sync: ${err.message}`);
    }
  } 
}
